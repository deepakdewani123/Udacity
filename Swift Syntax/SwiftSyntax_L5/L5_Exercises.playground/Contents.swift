...
func combineLastCharacter(name: [String]) -> String {
    var combinedArray = ""
    for myName in name {
        combinedArray.append(myName[myName.endIndex.predecessor()])
    }
    return combinedArray
    
}

combineLastCharacter(nonsenseArray)
//: __Problem 3__
//:
...
func removeAllFourLetterString(word: [String]) -> [String] {
    for myWord in word {
        if myWord.characters.count != 4 {
            cleanArray.append(myWord)
        }
    }
    return cleanArray
}
...
removeAllFourLetterString(dirtyWordsArray)
